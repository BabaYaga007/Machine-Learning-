...
yVec = zeros(m,num_labels); % 5000*10
for i=1:m
  yVec(i,y(i))=1;
end

% COST FUNCTION
cost = (1/m)* sum(sum((-yVec .* log(hypo)) - (1-yVec) .* log(1-hypo)));
J = cost;
...
for i = 1:m
    ...
    % Step 2
    del3 = a3 - yVec; % 5000*10
    
    % Step 3 
    del2 = (del3*Theta2) .* [ones(size(z2,1),1) sigmoidGradient(z2)]; %5000*26
    del2 = del2(:,2:end); % 5000*25
    
    % Step 4
    Theta1_grad = Theta1_grad + del2'*a1; % 25 x 401
    Theta2_grad = Theta2_grad + del3'*a2; % 10 x 26
...
regcost = (lambda/(2*m))*(sum(sum(regTheta1.^2)) + sum(sum(regTheta2.^2)));
J = J+regcost;
